...
class DB {
  ...
  Future _onCreate(Database db, int version) async {
    print('Creating');
    await db.execute(
        """CREATE TABLE $_bubble ($_bID INTEGER PRIMARY KEY AUTOINCREMENT, 
    ...
    await db.execute(
        """CREATE TABLE $_pop ($_pID INTEGER PRIMARY KEY AUTOINCREMENT, 
                        $_bID INTEGER NOT NULL, $_time_of_pop TEXT NOT NULL, $_action TEXT, 
                        FOREIGN KEY ($_bID) REFERENCES bubble($_bID))""");
    print('created pop');
    await db.execute(
        """CREATE TABLE $_app_state ($_loginID INTEGER PRIMARY KEY AUTOINCREMENT, 
                        $_last_opened TEXT)""");
    print('created app');
    await db.execute(
        """CREATE TABLE $_color_themes ($_colorID INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  Future<int> insertBubble(
    ...
    Map<String, dynamic> row = {
      ...
      '$_color_red': colorRed,
      '$_color_green': colorGreen,
      '$_color_blue': colorBlue,
      '$_opacity': opacity,
      '$_size': size,
      '$_posX': posX,
      '$_posY': posY,
      '$_time_created': new DateTime.now().toString(),
      '$_time_deleted': _not_deleted,
      '$_frequency': frequency,
      '$_days_to_repeat': days,
      '$_times_popped': 0
    };
    try {
      return await db.insert(_bubble, row);
    } catch (e) {
      print(e);
      return 0;
    }
  ...
  Future<Map<String, dynamic>> queryFullBubbleByID(int bID) async {
    Database db = await instance.database;
    try {
      return (await db.query(_bubble, where: '$_bID = ?', whereArgs: [bID]))
          .first;
    } catch (e) {
      print(e);
      return null;
  ...
  Future<Map<String, dynamic>> queryBubbleByID(
      int bID, List<String> col) async {
    Database db = await instance.database;
    try {
      return (await db.query(_bubble,
              columns: col, where: '$_bID = ?', whereArgs: [bID]))
          .first;
    } catch (e) {
      print(e);
      return null;
  ...
  Future<List<Map<String, dynamic>>> queryBubblesForRePop() async {
    Database db = await instance.database;
    try {
      final result = await db.query(_bubble,
          where: '$_time_deleted = ?', whereArgs: [_not_deleted]);
  ...
  Future<int> queryLastCreatedBubbleID() async {
    ...
    try {
      final results =
          await db.query(_bubble, columns: ['$_bID'], orderBy: '$_bID DESC');
  ...
  Future<int> updateBubbleByID(int bID, Map<String, dynamic> row) async {
    ...
    try {
      return await db
          .update(_bubble, row, where: '$_bID = ?', whereArgs: [bID]);
    } catch (e) {
      print(e);
      return 1;
    }
  ...
  Future<int> incrementBubbleTimesPopped(int bID) async {
    ...
    int currPop;
    try {
      currPop = (await db.query(_bubble,
              columns: ['$_times_popped'],
              where: '$_bID = ?',
              whereArgs: [bID]))
          .first['$_times_popped'];
    } catch (e) {
    ...
    try {
      return await db.update(_bubble, {'$_times_popped': currPop + 1},
  ...
  Future<bool> bubbleRepeatsToday(int bID) async {
    Database db = await instance.database;
    String currDay =
        dayToString(new DateTime.now().weekday); //Monday = 1, Sunday = 7
    String days;
    try {
      final result = (await db.query(_bubble,
              columns: ['$_bID, $_frequency, $_days_to_repeat'],
              where: '$_bID = ?',
              whereArgs: [bID]))
          .first;
      ...
      print('Unable to query $_days_to_repeat');
      return false;
    }
  ...
  Future<int> insertDelete(int bID) async {
    ...
    try {
      return await db.update(_bubble, {'$_time_deleted': time},
          where: '$_bID = ?', whereArgs: [bID]);
    } catch (e) {
      return 0;
    }
  ...
  Future<int> reverseDelete(int bID) async {
    Database db = await instance.database;
    try {
      return await db.update(_bubble, {'$_time_deleted': _not_deleted},
          where: '$_bID = ?', whereArgs: [bID]);
    } catch (e) {
      return 0;
    }
  ...
  Future<List<Map<String, dynamic>>> queryPopByBubble(int bID) async {
    ...
    try {
      return await db.query(_pop,
          where: '$_bID = ?', whereArgs: [bID], orderBy: '$_time_of_pop DESC');
    } catch (e) {
      print(e);
      return [];
    }
  ...
  Future<List<Map<String, dynamic>>> queryValidPopsByBubble(int bID) async {
    ...
    try {
      return await db.query(_pop,
          where: '$_bID = ? AND $_action = ?',
          whereArgs: [bID, _popped],
  ...
  Future<int> insertPop(int bID) async {
    ...
    Map<String, dynamic> row = {
      '$_bID': bID,
      '$_time_of_pop': new DateTime.now().toString(),
      '$_action': '$_popped'
    };
    try {
      return await db.insert(_pop, row);
    } catch (e) {
      print(e);
      return 0;
    }
  ...
  ///finds and changes the last pop made by a specific bubble and make the
  ///pop invalid an invalid pop will have 'UNPOPPED' as a value for the action
  ///field
  Future<int> undoPop(int bID) async {
    Database db = await instance.database;
    try {
      final pid = (await queryPopByBubble(bID)).first['$_pID'];
      return await db.update(_pop, {'$_action': '$_unpopped'},
          where: '$_pID = ?', whereArgs: [pid]);
    } catch (e) {
      return 0;
    }
  }
  ...
  Future<int> enterLogin(String time) async {
    Database db = await instance.database;
    try {
      return await db.insert(_app_state, {'$_last_opened': time});
    } catch (e) {
      print(e);
      return 0;
    }
  ...
  Future<String> latestLoginTime() async {
    Database db = await instance.database;
    try {
      String s = (await db.query(_app_state, orderBy: '$_last_opened DESC'))
          .first['$_last_opened'];
  ...
  Future<bool> login() async {
    ...
    if (lastTime == "") {
      enterLogin(new DateTime.now().toString());
      return true;
    } // first time login
  ...
  Future<List<Map<String, dynamic>>> queryAppState() async {
    Database db = await instance.database;
    try {
      return db.query(_app_state, orderBy: '$_last_opened DESC');
    } catch (e) {
      print(e);
      return null;
    }
  ...
  Future<List<Map<String, dynamic>>> queryColors() async {
    Database db = await instance.database;
    try {
      return await db.query(_color_themes);
    } catch (e) {
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>> queryColorsByID(int cID) async {
    ...
    try {
      return (await db
              .query(_color_themes, where: '$_colorID = ?', whereArgs: [cID]))
          .first;
    } catch (e) {
      return {};
    }
  ...
  void populateColorThemes() async {
    ...
    try {
      await db.insert(_color_themes, {
        _color_name: 'blue',
        _color_red: c.red,
        _color_green: c.green,
        _color_blue: c.blue,
        _opacity: c.opacity,
        _theme_name: 'Bubble'
      });
      ...
      print(e);
      print('Unable to populate blue');
    }

    try {
      c = Colors.deepOrange[200]; //Sunset Theme
      await db.insert(_color_themes, {
        _color_name: 'deepOrange',
        _color_red: c.red,
        _color_green: c.green,
        _color_blue: c.blue,
        _opacity: c.opacity,
      ...
      print(e);
      print('Unable to populate deepOrange');
    }
    ...
    try {
      ...
      print(e);
      print('Unable to populate purple');
    }
    ...
    try {
      ...
      print(e);
      print('Unable to populate yellow');
    }
    ...
    try {
      ...
      print(e);
      print('Unable to populate Ocean');
    }
  }
  ...
  File openXMLScript() {
    print('opening xml');
    if (_xmlFile != null) return _xmlFile;
  ...
  Future<xml.XmlDocument> createXML() async {
    try {
      ...
      if (xmlString.isNotEmpty) return xml.parse(xmlString);
    } catch (e) {
      print(e);
    }
  ...
  void printXML() {
    try {
      print(_settingsXML.toXmlString(pretty: true, indent: '\t'));
    } catch (e) {
      print(e);
    }
  }
  ...
  double getStoredFontSize() {
    var settings = _settingsXML
        .findElements(_settings)
