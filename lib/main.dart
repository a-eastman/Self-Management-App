...
class BubbleAppState extends State<BubbleApp>{
  ...
  Widget _buildBubbleView(){
    ...
    setState((){ _stack =_buildStack();});
    setState((){ _stack =_buildStack();});
    for (int i = 0; i < _stack.children.length; i++){
      BubbleWidget curWid = _stack.children[i] as BubbleWidget;
      print(curWid.getBubble().getEntry());
    }
    return Scaffold(
        appBar: AppBar(
          title: Text('BUBL'),
  ...
  Widget _buildStack(){
    List<BubbleWidget> _list = [];
    setState((){
      for (int i = 0; i < this._bList.getSize(); i++){
        Bubble curBub = this._bList.getBubbleAt(i);
        if (!curBub.getShouldDelete()){
          _list.add(new BubbleWidget(bubble: curBub));
        }
      }
    });
    Stack stack = new Stack(children: _list);
    return stack;
  }
  Widget _buildPages(){
