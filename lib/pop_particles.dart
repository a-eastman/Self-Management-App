...
///Main particle widget, will create a single burst of particles that
///disposes of itself after finishing
class PopParticles extends StatefulWidget {
  ...
  State<StatefulWidget> createState() =>
      PopParticlesState(this._bubble, _screenWidth, _screenHeight, "kee");
}
...
class PopParticlesState extends State<PopParticles> {
  ...
  PopParticlesState(
      this._bubble, this._screenWidth, this._screenHeight, this._key) {
    reset();
...
///Single node class for PopParticles, each individual particle is an
///instance of this
class PopParticleNode extends StatefulWidget {
  ...
  @override
  PopParticleNodeState createState() =>
      PopParticleNodeState(_bubble, _screenWidth, _screenHeight, _key);
}
...
class PopParticleNodeState extends State<PopParticleNode>
  ...
  PopParticleNodeState(
      this._bubble, this._screenWidth, this._screenHeight, this._key);

  ///Each particle ahs its own animation controller and handles its
  ///own animations
  ...
  ///Updates the animation controller and builds the state of the particle
  ///node widget using data from the animations
  @override
  Widget build(BuildContext context) {
        ...
        builder: (BuildContext context, Widget child) {
          return Transform(
            ...
            transform: Matrix4.translationValues(
              _baseXPos + (_xAnimation.value * _xExtents) - (screenWidth / 2.0),
              _baseYPos +
                  (_yAnimation.value * _yExtents) -
                  (screenHeight / 2.0),
              0.0,
            ),
              ...
              child: new Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: _bubble
                        .getColor()
                        .withAlpha((_bubble.getColor().alpha * .75).round()),
                  ),
                ),
              ),
            ),
  ...
  ///initState constructs each animation and multiplies the strength of its
  ///size/scale/etc by randomized values (tweakable above). This makes each
  ///particle move differently but follow the same pattern of floating up.
  ...
  void initState() {
    ...
    var xMult = randomBetween(rand, _minXMult, _maxXMult) * _screenHeight;
    var yMult = randomBetween(rand, _minYMult, _maxYMult) * _screenHeight;
    var shrinkTime = randomBetween(rand, _minShrinkTime, _maxShrinkTime);

    var bSize = _bubble.getSize() * _screenHeight;

    _baseSize = randomBetween(rand, _baseSizeMin, _baseSizeMax) * bSize;
